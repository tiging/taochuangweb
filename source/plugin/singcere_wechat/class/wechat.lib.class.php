...
class WeChatServer {
	...
	private function accessDataPush() {
		...
		if ($postdata) {
			if (!$this->_checkSignature()) {
				...
				require_once DISCUZ_ROOT . './source/plugin/singcere_wechat/class/notify.class.php';
				$this->_resclass = new wxNotifyResponse();
				return $this->_resclass->reply($postdata);
	...
	private function _activeHook($type) {
		...
		if(!isset($this->_resclass)) {
			require_once DISCUZ_ROOT . './source/plugin/singcere_wechat/class/response.class.php';
			$this->_resclass = new WxMpResponse();
		}
		
		return $this->_resclass->$hook($argvs);
	}
			...
			case 'event':
				...
				switch ($result['event']) {
					...
					case "masssendjobfinish": // 高级群发接口
						...
						break;
					case 'templatesendjobfinish':  // 推送模板消息发送状态
	...
	public static function getXml4CustomerService($account = '') {
    ...
    public static function getXml4Seccode($code, $title, $desc, $url) {
	    global $_G;
	    $codehash = base64_encode(authcode($code, 'ENCODE', $_G['config']['security']['authkey'], 600));
	    return WeChatServer::getXml4RichMsgByArray(array(array(
	        'title' => $title,
	        'desc' => $desc,
	        'pic' => $_G['siteurl'].'plugin.php?id=singcere_wechat:misc&ac=seccode&codehash='.urlencode($codehash),
	        'url' => $url,
	    )));
	}
...
class WeChatClient {
	...
	public function getJsApiTicket($ticketonly = 1, $nocache = 0) {
	    ...
	    if (!empty(self::$_jsApiTicketCache[$appid])) {
	        ...
	        if (time() < $myTicketInfo['expiration'] - 200) {
	            return $ticketonly ? $myTicketInfo['ticket'] : $myTicketInfo;
	        }
	    }
	    
	    $access_token = $this->getAccessToken();
	    $url = self::$_URL_API_ROOT . "/cgi-bin/ticket/getticket?type=1&access_token=$access_token";
	...
	public function getAccessToken($tokenOnly = 1, $nocache = 0) {
		...
		if (!empty(self::$_accessTokenCache[$appid])) {
			...
			if (time() < ($myTokenInfo['expiration'] - 200)) {
	...
	public function getCustomerServiceList() {
	    $access_token = $this->getAccessToken();
	    $url = self::$_URL_API_ROOT . "/cgi-bin/customservice/getonlinekflist?access_token=$access_token";
	...
	public function setMenu($myMenu, $conditional = false) {
		$access_token = $this->getAccessToken();
		
		$url = self::$_URL_API_ROOT . "/cgi-bin/menu/".($conditional ? 'addconditional' : 'create')."?access_token=$access_token";
	...
	public function sendTextMsg($to, $msg) {
		$msg = urlencode(diconv($msg, CHARSET, 'UTF-8'));
    ...
    public function sendTemplateMsg($tmpl_id, $to, $detail_url, $topcolor, $data) {
	    $access_token = $this->getAccessToken();
	    $url = self::$_URL_API_ROOT . "/cgi-bin/message/template/send?access_token=$access_token";
	    $data = sc_diconv($data, CHARSET, 'UTF-8');
...
class WeChatEmoji {
	...
	public static function clear($str) {
		...
		if(CHARSET == "utf-8") {
			$str = diconv(diconv($str, 'utf-8', 'gbk'), 'gbk', CHARSET);
		} else {
			$str = diconv($str, 'UTF-8', CHARSET);
		}
		return $str; 
	}
...
class WeChatHook {
	...
	public static function bindOpenId($uid, $openid, $isregister = 0) {
		C::t('#singcere_wechat#singcere_wechat_bind')->insert(array('uid' => $uid, 'openid' => $openid, 'isregister' => $isregister), false, true);
	}
