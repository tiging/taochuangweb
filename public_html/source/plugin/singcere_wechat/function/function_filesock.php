...
function sc_dfsockopen($url, $limit = 0, $post = '', $cookie = '', $bysocket = FALSE, $ip = '', $timeout = 15, $block = TRUE, $encodetype  = 'URLENCODE', $allowcurl = TRUE, $position = 0, $files = array()) {
	$return = '';
	...
	if($post) {
		...
		sc_format_postkey($post, $postnew);
		$post = $postnew;
	}
	...
	$fpflag = 0;
	if(!$fp = @fsocketopen(($ip ? $ip : $host), $port, $errno, $errstr, ($timeout ? $timeout : 15))) {
		$context = array(
			'http' => array(
...
function sc_format_postkey($post, &$result, $key = '') {
	foreach($post as $k => $v) {
		$_k = $key ? $key.'['.$k.']' : $k;
		if(is_array($v)) {
			sc_format_postkey($v, $result, $_k);
		} else {
			$result[$_k] = $v;
		}
	}
